...
const RayTracerCanvas: React.FC<Props> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if (document.pointerLockElement === canvasRef.current) {
      if (e.button === 0) performRaycastAction(false);
      if (e.button === 2) performRaycastAction(true);
    } else {
  ...
  const performRaycastAction = (isRightClick: boolean = false) => {
    ...
    if (closestLightIdx !== -1 && minLightDist < voxelDist) {
      onLightClick(closestLightIdx, isRightClick);
    } else if (hit) {
      onBlockAction(hit.pos, hit.normal, isRightClick);
    }
  };
